...
- The embedded Express API serves OpenAPI at `{apiBaseUrl}/api/openapi.json` and Swagger UI at `{apiBaseUrl}/api/docs` (loopback only); `pnpm dev` logs the Swagger URL next to the API base line in main (`bootstrap.ts`). Keep `/api/*` mounts, OpenAPI `paths`, and typed HTTP clients aligned via `src/shared/api/embedded-api-paths.ts`; after changing paths run `pnpm codegen:api` (CI uses `pnpm codegen:api:check`).
