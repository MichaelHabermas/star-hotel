...
- The Home route (`/` / `home-page.tsx`) renders the **Developer experience** card (embedded API health, IPC, perf smoke, list reservations) only when `import.meta.env.DEV` is true; production and packaged builds omit it. A separate dev-only **Dev error** test route remains for US1.3 / global error boundary smoke.
- Create-mode reservation prefill from URL search params (`useReservationEditor`): sync `guestId`, `roomId`, and stay dates from `initial*` values in `useEffect` when those inputs change—`useState` initializers only run on mount, so in-place navigation between rooms can leave stale picks otherwise.
- Room board reservation snapshots (`room-dashboard` / `buildReservationSnapshots`): the calendar “today” string used for active vs upcoming stays must advance on long-lived mounts (e.g. state refreshed on an interval), not a one-time `useMemo` with an empty dependency array, or desk actions drift after midnight. If Electron main fails to load `better-sqlite3` or `argon2` with a NODE_MODULE_VERSION / ABI mismatch, rerun `pnpm rebuild:native` or `pnpm install` (postinstall runs `electron-rebuild` for both). `pnpm test` rebuilds those modules for **system Node**; `posttest` runs `electron-rebuild` so `pnpm dev` still works after a test run.
...
- In **dev** (`electron-vite serve`), the embedded API serves OpenAPI at `{apiBaseUrl}/api/openapi.json` and Swagger UI at `{apiBaseUrl}/api/docs` (loopback only); `pnpm dev` logs the Swagger URL in main (`bootstrap.ts`). **Packaged** builds omit those routes (`import.meta.env.STAR_HOTEL_INCLUDE_OPENAPI`). Keep OpenAPI `paths` and typed HTTP clients aligned via `src/shared/api/embedded-api-paths.ts`; after changing paths run `pnpm codegen:api` (CI uses `pnpm codegen:api:check`).
- `DEV_SEED_RESERVATIONS_ANCHOR_DATE` in `src/shared/constants.ts` anchors dev seeding and report defaults—keep it aligned with test fixtures and the intended calendar year so day-sheet and reservation tests do not drift.
- Prettier uses `prettier-plugin-organize-imports` (listed in `package.json` devDependencies and `.prettierrc`); if it is removed, `pnpm format` and `pnpm cleanup` fail. Do not add `importOrder*` options meant for other sort-imports plugins (e.g. `@ianvs/prettier-plugin-sort-imports`)—Prettier ignores them and warns once per file. Prettier 3 supports `--log-level error` to quiet non-error log lines (including those warnings); `--check` still exits non-zero when files differ.
...
- Main and preload are **bundled** by electron-vite (`build.externalizeDeps: false`); Rollup externals are native addons (`better-sqlite3`, `argon2`), `electron`, and Node builtins. OpenAPI/Swagger are loaded only when `STAR_HOTEL_INCLUDE_OPENAPI` is true (`pnpm dev` / Vitest), so production `out/main` does not include `swagger-ui-*`. Changing externals or the OpenAPI flag requires a deliberate change in [`electron.vite.config.ts`](electron.vite.config.ts) / [`vitest.config.ts`](vitest.config.ts).
