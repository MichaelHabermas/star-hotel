...
- `.prettierrc` lists `prettier-plugin-organize-imports` in `plugins`; add that package as a devDependency (or remove it from the Prettier config). Without it, `pnpm format` and `pnpm cleanup` can fail when Prettier loads plugins.
