...
import { DEV_FAKE_RESERVATION_COUNT, seedDevReservationsIfNeeded } from './seed-dev-reservations';
...
describe('seedDevReservationsIfNeeded', () => {
  ...
  it('inserts baseline rooms, guests, and the target reservation count once', () => {
    ...
    const resCount = (
      db.prepare('SELECT COUNT(*) AS c FROM tbl_reservation').get() as { c: number }
    ).c;
